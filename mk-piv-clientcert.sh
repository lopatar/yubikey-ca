#!/usr/bin/env bash
# mk-piv-clientcert.sh — generate & sign TLS client certificate (clientAuth) using YubiKey PIV (slot 9c) with ECC
# Usage: ./mk-piv-client-cert.sh <CN> [EMAIL] [CURVE]
set -euo pipefail
...
print_usage() {
  cat <<'EOF'
Usage:
  mk-piv-clientcert.sh <CN> [EMAIL] [CURVE]
